      ...
      "zgsp26.conf.mng.sdprice.confmngfesdprice.ext.main.Main",
      {
        ...
        _getExcelImportConfig: function () {
          return {
            ...
            boolFields: [],
            numericFields: ["MaxDiscount", "MinOrderVal"],
          ...
          this._ensureXlsxLoaded().then(function () {
            ...
            oInput.addEventListener("change", function (oEvent) {
              ...
              if (oFile.size > MAX_FILE_SIZE) {
                MessageBox.error("File size exceeds 5 MB limit. Please use a smaller file.");
        ...
        _processExcelData: function (arrayBuffer) {
          ...
          if (result.rows.length > 1000) {
            MessageBox.error("The file contains " + result.rows.length + " rows, exceeding the 1,000-row limit. Please split the file.");
            return;
          }
        ...
        _insertImportedRows: function (result) {
          ...
          // Validate ALL rows (imported + existing) — catches duplicates cross-set
          var aErrors = this._validateRows(aRows);
          ...
          if (aErrors.length) {
            MessageBox.warning(
              sMsg + "\n\n" + aErrors.length + " validation issue(s) found. Please review highlighted rows before saving.",
        ...
        _validateRows: function (aRows) {
          ...
          aRows.forEach(function (row, iIdx) {
            ...
            // Explicit mandatory checks (always enforced regardless of field def)
            if (!String(row.BranchId ?? "").trim()) {
              row._vs_BranchId = "Error";
              aErrors.push(sRowNum + ": Branch is required.");
            }
            if (!String(row.CustGroup ?? "").trim()) {
              row._vs_CustGroup = "Error";
              aErrors.push(sRowNum + ": Customer Group is required.");
            }
            if (!String(row.Currency ?? "").trim()) {
              row._vs_Currency = "Error";
              aErrors.push(sRowNum + ": Currency is required.");
            }
            ...
            } else if (!isNaN(nDisc) && nDisc > 100) {
              row._vs_MaxDiscount = "Error";
              aErrors.push(sRowNum + ": Max Discount cannot exceed 100%.");
            }
          ...
          // ── Duplicate business key check (BranchId + CustGroup + MaterialGrp) ──
          const aSeen = [];
          aRows.forEach(function (row, iIdx) {
            if (row._state === "deleted") return;
            const sBranch   = String(row.BranchId    || "").trim();
            const sCustGrp  = String(row.CustGroup   || "").trim();
            const sMatGrp   = String(row.MaterialGrp || "").trim();
            if (!sBranch || !sCustGrp || !sMatGrp) return;

            const sKey = sBranch + "|" + sCustGrp + "|" + sMatGrp;
            if (aSeen.indexOf(sKey) >= 0) {
              row._vs_BranchId    = "Error";
              row._vs_CustGroup   = "Error";
              row._vs_MaterialGrp = "Error";
              aErrors.push("Row " + (iIdx + 1) + ": Duplicate entry — Branch '" + sBranch + "' + Customer Group '" + sCustGrp + "' + Material Group '" + sMatGrp + "' already exists in this request.");
            } else {
              aSeen.push(sKey);
            }
          });
